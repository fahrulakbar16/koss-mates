<template>
    <div class="relative" ref="dropdownWrapper">
        <!-- Tombol -->
        <button :id="buttonId" @click="toggleDropdown" :disabled="props.disabled" type="button"
            :class="['w-full rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 bg-white dark:bg-gray-700 dark:text-white dark:border-gray-600 flex justify-between items-center', props.inputClass]">
            <span class="truncate">
                {{ selectedLabel }}
            </span>
        ...
        <teleport to="body">
            <div v-show="open" :style="dropdownStyle" ref="dropdownContent"
                ...
                <ul class="max-h-24 overflow-y-auto" data-simplebar>
                    <!-- Clear/Reset Option -->
                    <li class="px-4 py-2 cursor-pointer flex items-center gap-2 hover:bg-gray-100 dark:hover:bg-gray-600 border-b border-gray-200"
                        @click="clearSelection">
                        <input type="radio" class="text-blue-600" :checked="!modelValue || modelValue === null"
                            @change.stop />
                    ...
                    <li v-for="item in filteredItems" :key="item.id"
                        ...
                        <input type="radio" class="text-blue-600" :checked="isSelected(item)" @change.stop />
                    ...
                    <li v-if="!taggable && filteredItems.length === 0" class="px-4 py-2 text-gray-500 italic">
                        Tidak ada hasil
                    </li>
                </ul>
            </div>
...
function handleClickOutside(e) {
    const isInsideWrapper = dropdownWrapper.value && dropdownWrapper.value.contains(e.target);
    const isInsideContent = dropdownContent.value && dropdownContent.value.contains(e.target);
