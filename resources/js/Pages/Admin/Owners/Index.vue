    ...
    <div class="flex flex-col gap-6 px-4 sm:px-6 lg:px-8 py-8 h-full">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            <div>
                <div class="flex items-center gap-3 mb-3">
                    ...
                    <h1 class="text-3xl sm:text-4xl font-bold text-gray-900 dark:text-white">Akun Pemilik</h1>
                </div>
                <p class="text-gray-600 dark:text-gray-400 text-base ml-5">
                    Kelola akun pemilik kos dan informasi rekening
                </p>
            </div>
        ...
        <div class="h-full flex flex-col overflow-hidden rounded-2xl bg-white dark:bg-gray-800 shadow-xl shadow-gray-200/50 dark:shadow-none border border-gray-100 dark:border-gray-700">
            ...
            <div class="p-5 border-b border-gray-100 dark:border-gray-700 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div class="flex items-center gap-2 w-full sm:w-auto">
                    <div class="relative w-full sm:w-64">
                        ...
                        <input v-model="searchQuery" type="text" placeholder="Cari pemilik..."
            ...
            <div class="overflow-x-auto pb-8 flex-1" data-simplebar>
                <div v-if="owners.data && owners.data.length > 0">
                    <table class="w-full text-left border-collapse min-w-[1000px]">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-800 bg-white dark:bg-gray-900">
                            <tr v-for="owner in owners.data" :key="owner.id"
                                ...
                                <td class="px-6 py-5 whitespace-nowrap">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <div v-if="owner.username" class="text-xs text-gray-500 dark:text-gray-400">
                                                @{{ owner.username }}
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <td class="px-6 py-5 whitespace-nowrap">
                                    <div class="text-sm text-gray-900 dark:text-white">{{ owner.email }}</div>
                                </td>
                                <td class="px-6 py-5 whitespace-nowrap">
                                    <div v-if="owner.owner" class="flex flex-col gap-1">
                                        <div class="text-sm font-bold text-gray-900 dark:text-white">
                                            {{ owner.owner.bank_name || '-' }}
                                        </div>
                                        <div class="text-xs text-gray-500 dark:text-gray-400">
                                            {{ owner.owner.bank_account_number || '-' }}
                                            <span v-if="owner.owner.bank_account_name">({{ owner.owner.bank_account_name }})</span>
                                        </div>
                                    </div>
                                    <div v-else class="text-sm text-gray-400 italic">Belum ada info bank</div>
                                </td>
                                <td class="px-6 py-5 whitespace-nowrap text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        <Link :href="route('admin.owners.edit', owner.id)"
                                        ...
                                        <button @click="confirmDelete(owner)"
                ...
                <div v-else
                    ...
                    <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-2">Belum ada Pemilik</h3>
                    <p class="text-gray-500 dark:text-gray-400 max-w-sm mx-auto mb-6">
                        Belum ada data pemilik kos yang ditemukan. Silakan tambahkan pemilik baru.
                    </p>
                    <Link :href="route('admin.owners.create')"
                        class="inline-flex gap-2 items-center px-5 py-2.5 text-white rounded-xl bg-gradient-to-r from-primary-600 to-primary-500 hover:from-primary-700 hover:to-primary-600 shadow-lg shadow-primary-500/30 hover:shadow-primary-500/40 transition-all duration-300 transform hover:-translate-y-0.5">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <!-- Pagination -->
            <div v-if="owners.data && owners.data.length > 0" class="border-t border-gray-100 dark:border-gray-700 p-4">
                <Pagination :pagination="owners" />
            </div>
        </div>
        ...
        <div v-if="showDeleteModal"
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-700 max-w-md w-full p-8 transform transition-all scale-100">
                ...
                <p class="text-center text-sm text-gray-500 dark:text-gray-400 mb-6 leading-relaxed">
                    Apakah Anda yakin ingin menghapus akun <strong>{{ ownerToDelete?.name }}</strong>?
...
function deleteOwner() {
    if (ownerToDelete.value) {
        router.delete(route('admin.owners.destroy', ownerToDelete.value.id), {
            onSuccess: () => {
